...
import com.yessinmedqa.models.Reponse;
import com.yessinmedqa.services.ModerationService;
import com.yessinmedqa.services.QuestionService;
import com.yessinmedqa.services.ReponseService;
...
public class QuestionsRepController {
    ...
    private void retourEspaceMedecin() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource(
                    "/com/yessinmedqa/espaceMedecin.fxml"));
    ...
    private VBox buildCard(Reponse rep) {
        ...
        if (allReponses.isEmpty()) {
            ...
            // ✅ Show each response in its own block
            for (Reponse r : allReponses) {
                ...
                Label doctorName = new Label("Dr " + r.getDoctorPrenom()
                        + " " + r.getDoctorNom());
                ...
                // ✅ Edit button — only for the doctor who wrote THIS response
                if (Session.isDoctor() && r.getDoctorId() == Session.getUserId()) {
                    ...
                    saveBtn.setOnAction(e -> {
                        ...
                        reponseService.updateReponse(
                                r.getId(), Session.getUserId(), newContenu);
        ...
        if (Session.isDoctor()) {
            ...
            addBtn.setOnAction(e -> {
                addAnswerBox.setVisible(true);
                addAnswerBox.setManaged(true);
            ...
            submitNewBtn.setOnAction(e -> {
                ...
                reponseService.addAnotherReponse(
                        rep.getQuestionId(), Session.getUserId(), contenu);
    ...
    private void ouvrirReponse(Reponse rep) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource(
                    "/com/yessinmedqa/voirReponse.fxml"));
    ...
    public void filtrerCategorie(ActionEvent event) {
        ...
        if (text.equals("Toutes les catégories")) {
            chargerReponses("", null);
        } else {
            chargerReponses("", text);
        }
    }
    ...
    public void allerPoserQuestion(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource(
                    "/com/yessinmedqa/poserQuestion.fxml"));
            searchField.getScene().setRoot(root);
        } catch (IOException e) {
            System.err.println("Erreur navigation: " + e.getMessage());
        }
    ...
    private void supprimerQuestion(Reponse rep) {
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
        confirm.setTitle("Confirmation");
        confirm.setHeaderText("Supprimer cette question ?");
        confirm.setContentText("Cette action est irréversible.");
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                reponseService.deleteQuestion(rep.getQuestionId());
                chargerReponses(searchField.getText().trim(), null);
            }
        });
    }
