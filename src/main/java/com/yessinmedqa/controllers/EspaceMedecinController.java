...
import com.yessinmedqa.services.AIService;
...
import com.yessinmedqa.services.RecommandationService;
...
public class EspaceMedecinController {
    ...
    private boolean isMySpecialite(Question q) {
        ...
        if (doctorSpec == null) return true; // no speciality = sees all
        if (q.getCategorie() == null) return true; // no category = sees all

        String requiredSpec = CATEGORY_MAP.get(q.getCategorie());
        if (requiredSpec == null) return true; // unknown category = sees all
    ...
    private void showQuestionReportDialog(Question q) {
        ...
        sendBtn.setOnAction(e -> {
            ...
            new com.yessinmedqa.services.ModerationService()
                    .reportQuestion(q.getId(), Session.getUserId(), raison);
    ...
    public void voirQuestionsRepondues(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource(
                    "/com/yessinmedqa/questionsRepondues.fxml"));
